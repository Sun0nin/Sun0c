const  XBOT = 'ꪶ⚘࿆ꦿི ݈݇Sun0Bot ⃝༘⃕🍒'; // Nama Bot Whatsapp
const instagram = 'Desativado'; // Nama Instagramlu cok
const nomer = 'wa.me/+17542478646'; // Nomor whatsapplu cok
const aktif = 'Em testes'; // Kapan bot lu aktif
const groupwa = 'https://chat.whatsapp.com/IlwIus5EuRi4BScxeNmhJE'; // OFFICIAL GRUP LU 1
const youtube = 'https://youtube.com/channel/UCx7J37pn313Q1fYiSZdx30Q'; 
...
const vcard = 'BEGIN:VCARD\n' // metadados do cartão de contato
            + 'VERSION:3.0\n' 
            + 'FN:affis\n' // full name
  ...
  var teks = text.replace(/#nulis /, '')
    ...
    .then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            conn.sendMessage(id, 'Buscando...🔍', MessageType.text)
...
axios.get(`https://st4rz.herokuapp.com/api/yta?url=${teks}`).then((res) => {
    let hasil = `O áudio está disponível no link abaixo, clique no link e baixe os resultados\n👇👇👇👇👇👇👇👇👇\n\nJudul: ${res.data.title}\n\nUkuran audio: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    ...
    axios.get(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=${bg}`).then((res) => {
      imageToBase64(res.data.result)
        .then(
          (ress) => {
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, 'Em andamento🔍 por favor, aguarde um momento', MessageType.text, { quoted: m })
...
axios.get(`https://st4rz.herokuapp.com/api/ytv?url=${teks}`).then((res) => {
    let hasil = `O vídeo está disponível no link abaixo, clique no link e baixe os resultados\n\n\nJudul: ${res.data.title}\n\nTamanho do vídeo: ${res.data.filesize}\n\nLink: ${res.data.result}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://api.terhambar.com/negara/World`).then((res) => {
	conn.sendMessage(id, '[WAIT] Procurando...🔍', MessageType.text)
    let hasil = `info corona all \n\n *país* : _${res.data.negara}_ \n *total* : _${res.data.total}_ \n *novos casos* : _${res.data.kasus_baru}_ \n *mortes* : _${res.data.meninggal}_ \n *mortes_ novas* : _${res.data.meninggal_baru}_ \n *curados* : _${res.data.sembuh}_ \n *tratamento* : _${res.data.penanganan}_ \n *últimos* : _${res.data.terakhir}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://docs-jojo.herokuapp.com/api/mostviewfilm`).then((res) => {
	conn.sendMessage(id, '[WAIT] Procurando...🔍', MessageType.text)
    let hasil = `resultados da mostviewfilm :) \n\n${res.data.title} \n *rank* ${res.data.rank} \n *penonton* ${res.data.penonton}`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
...
axios.get(`https://arugaz.herokuapp.com/api/howgay`).then((res) => {
	conn.sendMessage(id, '[WAIT] Processando...🔍', MessageType.text)
    let hasil = ` ${res.data.desc} \n\n *Porcentagem Gay LoL!!!* _${res.data.persen}_`;
    conn.sendMessage(id, hasil ,MessageType.text);
})
}
    ...
    axios.get(`http://inyourdream.herokuapp.com/glitch?kata1=${teks1}&kata2=${teks2}`).then((res) => {
      imageToBase64(res.data.status)
        .then(
          (ress) => {
            var buf = Buffer.from(ress, 'base64')
            conn.sendMessage(id, '[ WAIT ] SENDO PROCESSADO🔍 silahkan tunggu sebentar', MessageType.text, { quoted: m })
